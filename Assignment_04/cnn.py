...
class CNN(object):
    ...
    def append_dense_layer(self, num_nodes, activation="relu", name="", trainable=True):
        """
         ...
         :return: None
         """
        self.model.add(tf.keras.layers.Dense(num_nodes, activation=activation, name=name, trainable=trainable))
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        return self.model.add(
            tf.keras.layers.Conv2D(filters=num_of_filters, kernel_size=kernel_size, padding=padding, strides=strides,
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2, name=""):
        """
         ...
         """
        return self.model.add(
            tf.keras.layers.MaxPool2D(pool_size=pool_size, padding=padding, strides=strides, name=name))
    ...
    def append_flatten_layer(self, name=""):
        """
         ...
         :return: Layer object
         """
        return self.model.add(tf.keras.layers.Flatten(name=name))
    ...
    def set_training_flag(self, layer_numbers=[], layer_names="", trainable_flag=True):
        ...
        if isinstance(layer_numbers, list):
            for i in range(len(layer_numbers)):
                self.model.get_layer(index=i, name=layer_names[i]).trainable = trainable_flag

        else:
            self.model.get_layer(index=layer_numbers, name=layer_names).trainable = trainable_flag
    ...
    def get_weights_without_biases(self, layer_number=None, layer_name=""):
        ...
        if layer_number is not None:
            ...
            if len(self.model.get_layer(name=layer_name).get_weights()) < 1:
                return None
    ...
    def set_weights_without_biases(self, weights, layer_number=None, layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        if layer_number:
            tf.keras.backend.set_value(self.model.get_layer(index=layer_number - 1).weights[0], weights)

        else:
            tf.keras.backend.set_value(self.model.get_layer(name=layer_name).weights[0], weights)
    ...
    def set_biases(self, biases, layer_number=None, layer_name=""):
        ...
        """
        if layer_number:
            tf.keras.backend.set_value(self.model.get_layer(index=layer_number - 1).weights[1], biases)

        else:
            tf.keras.backend.set_value(self.model.get_layer(name=layer_name).weights[1], biases)
    ...
    def remove_last_layer(self):
        ...
        self.model = tf.keras.Sequential(self.model.layers)
    ...
    def load_a_model(self, model_name="", model_file_name=""):
        ...
        if model_name:
            if model_name.lower() == "vgg16":
                self.model = tf.keras.Sequential(tf.keras.applications.VGG16().layers)
            else:
                self.model = tf.keras.Sequential(tf.keras.applications.VGG19().layers)
